...
MaRQ_results = open(sys.argv[1], "r")

current_bgp = set()
l_bgp = []
S2S = False
for line in MaRQ_results.readlines():
    if line[0] == "M":      # print the name of the mappings
        print(line[:-1])

    if line == 'S-S\n':     # select the part that we wanna test
        S2S = True
    if line == 'O-O\n':
        S2S = False
...
for classes in l_bgp:
    ...
    print(classes - ({v for k in classes for v in superclasses[k]}))
